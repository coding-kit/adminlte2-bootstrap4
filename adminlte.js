...
window.slugGenerate = function slugGenerate(text, slugId) {
    var slug = text.trim().slugify();
    $(`#${slugId}`).val(slug);
}
...
window.deleted = function deleted(url) {
    Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.value) {
            var deleteForm = `<form method="POST" action="`+url+`" id="deleteForm">
...
/* file-input:START */
window.readUpload = function readUpload(input, id) {
    if (input.files && input.files[0]) {
        var reader = new FileReader();
        reader.onload = function(e) {
            $(`#${id} .image-upload-wrap`).hide();
            $(`#${id} .file-upload-image-box`).html(`<img src="${e.target.result}" class="img-fluid" />`);
            $(`#${id} .file-upload-content`).show();

            $(`#${id}_is_removed`).val(0);
        };
        reader.readAsDataURL(input.files[0]);
    } else {
        removeUpload();
...
window.removeUpload = function removeUpload(id) {
    $(`#${id}_is_removed`).val(1);
    $(`#${id}_file`).val(null);
